...
fileprivate struct AndroidSDKTests {
    ...
    @Test func debian() async throws {
        let fs = PseudoFS()
        let sdkPath = Path("/usr/lib/android-sdk")
        try fs.createDirectory(sdkPath, recursive: true)
        try await withNDKVersions(fs: fs, sdkPath: sdkPath, versions: Version("24"), Version("25"), Version("26")) { host, fs, sdkPath, ndkVersionPaths in
            ...
            try fs.symlink(Path("/usr/lib/android-ndk"), target: ndkVersionPaths[1])
    ...
    private func withNDKVersions(fs: PseudoFS = PseudoFS(), sdkPath: Path = .root, versions: Version..., block: (OperatingSystem, any FSProxy, Path, [Path]) async throws -> ()) async throws {
        let ndkPath = sdkPath.join("ndk")
        ...
        let host = try ProcessInfo.processInfo.hostOperatingSystem()
        try await block(host, fs, sdkPath, ndkVersionPaths)
    }

    private func withNDKVersion(fs: PseudoFS = PseudoFS(), sdkPath: Path = .root, version: Version, _ block: (OperatingSystem, any FSProxy, Path, Path) async throws -> ()) async throws {
